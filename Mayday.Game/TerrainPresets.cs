...
namespace Mayday.Game
{
	...
	public class TerrainPresets
	{
		...
		public static ModuleBase GetPreset(PresetType preset)
		{
			switch (preset)
			{
				case PresetType.mountains: return Mountains();
				case PresetType.caves: return Caves();
				case PresetType.cavesAndMountains: return CavesAndMountains();
				case PresetType.fractal: return FractalExample();
			}

			return CavesAndMountains();
		}
		...
		public static ModuleBase Mountains()
		{
			...
			TranslatedDomain ground_overhang_perturb =
				new TranslatedDomain(ground_perturb, ground_overhang_scale, null);
		...
		{
			Fractal cave_shape = new Fractal(FractalType.RIDGEDMULTI, BasisTypes.GRADIENT, InterpTypes.QUINTIC, 1, 2,
				4533);
			Select cave_select = new Select(cave_shape, 1, 0, 0.6, 0);

			Fractal cave_perturb_fractal =
				new Fractal(FractalType.FBM, BasisTypes.GRADIENT, InterpTypes.QUINTIC, 6, 3, null);
		...
		public static ModuleBase CavesAndMountains()
		{
			...
			Fractal lowland_shape_fractal =
				new Fractal(FractalType.BILLOW, BasisTypes.GRADIENT, InterpTypes.QUINTIC, 2, 0.3, null);
			...
			Fractal terrain_type_fractal =
				new Fractal(FractalType.FBM, BasisTypes.GRADIENT, InterpTypes.QUINTIC, 3, 0.125, null);
			...
			//
			// // caves
			// Fractal cave_shape = new Fractal(FractalType.RIDGEDMULTI, BasisTypes.GRADIENT, InterpTypes.QUINTIC, 1, 4,
			// 	null);
			...
			return DigCaves(ground_select);
		}

		public static ModuleBase CreateOres(ModuleBase moduleBase)
		{
			Fractal cave_shape = new Fractal(FractalType.RIDGEDMULTI, BasisTypes.GRADIENT, InterpTypes.QUINTIC, 1, 4,
				null);
			Bias cave_attenuate_bias = new Bias(moduleBase, 0.9);
			Combiner cave_shape_attenuate = new Combiner(CombinerTypes.MULT, cave_shape, cave_attenuate_bias);
			Fractal cave_perturb_fractal =
				new Fractal(FractalType.FBM, BasisTypes.GRADIENT, InterpTypes.QUINTIC, 6, 3, null);
			ScaleOffset cave_perturb_scale = new ScaleOffset(0.2, 0, cave_perturb_fractal);
			TranslatedDomain cave_perturb = new TranslatedDomain(cave_shape_attenuate, cave_perturb_scale, null);
			Select cave_select = new Select(cave_perturb, 1, 0, 0.05, 0);
			return new Combiner(CombinerTypes.MULT, moduleBase, cave_select);
		}

		public static ModuleBase DigCaves(ModuleBase moduleBase)
		{
